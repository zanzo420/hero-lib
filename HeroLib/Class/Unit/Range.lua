...
--- ============================ CONTENT ============================
-- Construct IsInRangeTable based on the ItemRange Enum generated manually by FilterItemRange
...
function Unit:IsInRange(Distance, AoESpell)
  ...
  if DistanceType == "number" or (DistanceType == "string" and Distance == "Melee") then
    ...
    if DistanceType == "number" then
      ...
      if AoESpell then Distance = Distance - 1.5 end
      ...
      if not ItemRange[Distance] then
        ...
        if not ItemRange[Distance] then Distance = "Melee" end
      end
    end
    return IsItemInRange(ItemRange[Distance], self.UnitID)
  -- Distance check through IsSpellInRange (works only for targeted spells only)
  elseif DistanceType == "table" then
    return IsSpellInRange(Distance:Name(), self.UnitID) == 1
  else
